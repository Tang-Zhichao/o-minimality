...
\begin{definition}
  ...
  $\{\phi(m_1,\dots,m_n):\phi\text{ is either an atomic $\mathcal{L}$-formula or the nagation of an atomic $\mathcal{L}$-formula and $\mathcal{M} \models \phi(m_1,\dots,m_n)$}\}$.
...
\begin{definition}
    ...
    Icomplete types are refered to as \emph{partial types}.
    We often write $p(v_1,\dots,v_n)$ to stree that $p$ is an $n$-type.
...
\begin{theorem}[Omitting Types Theorem]
  ...
  and $p$ a (possibly incomplete) nonisolated $n$-type over $\emptyset$.
